...
import { Search, LayoutGrid, Sidebar, Loader2, Webhook, Globe, Users, Plug, Database, X, Plus, Check, MousePointerClick, type LucideIcon } from "lucide-react";
            ...
            <AnimatePresence mode="wait">
                {isMinimized ? (
                    ...
                    >
                        ...
                        {/* Click-to-add hint in subagent mode */}
                        {isSubagentMode && (
                            <div className="flex items-center gap-1.5 px-5 pb-2">
                                <MousePointerClick className="w-3 h-3 text-neutral-400 canvas-dark:text-neutral-500" strokeWidth={2} />
                                <span className="text-[10px] text-neutral-400 canvas-dark:text-neutral-500">Click to add</span>
                            </div>
                        )}
                                            ...
                                            <>
                                                {filteredCustom.map((t) => (
                                                    <motion.div key={t.type} variants={STAGGER_ITEM}>
                                                        ...
                                                        >
                                                            {isSubagentMode && (
                                                                <div className="absolute top-1.5 right-1.5 w-5 h-5 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity z-10" style={{ background: "linear-gradient(135deg, #FF8C00, #9D50BB)" }}>
                        ...
                        {/* Sticky Done button for subagent mode */}
                        {isSubagentMode && (
